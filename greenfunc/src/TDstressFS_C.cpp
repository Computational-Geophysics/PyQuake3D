...
py::array_t<double> TDstressFS_py(
    ...
    double lambda_)
{
    std::vector<double> Sxx, Syy, Szz;
    std::vector<double> Sxy, Sxz, Syz;

    std::vector<double> Exx, Eyy, Ezz;
    std::vector<double> Exy, Exz, Eyz;
    ...
    //const size_t N = Sxx.size();

    py::ssize_t N = static_cast<py::ssize_t>(Sxx.size());

    py::array_t<double> stress(
        std::vector<py::ssize_t>{N, 6}
    );
    
    auto s = stress.mutable_unchecked<2>();
...
py::array_t<double> TDstressHS_py(
    const std::vector<double>& X,
    const std::vector<double>& Y,
    const std::vector<double>& Z,
    const std::vector<double>& P1,
    ...
    double lambda_)
{
    ...
    auto stress_view = stress.mutable_unchecked<2>();

    for (ssize_t i = 0; i < N; ++i)
    {
        stress_view(i, 0) = Sxx[i];
        stress_view(i, 1) = Syy[i];
        stress_view(i, 2) = Szz[i];
        stress_view(i, 3) = Sxy[i];
        stress_view(i, 4) = Sxz[i];
        stress_view(i, 5) = Syz[i];
    }

    return stress;
}

PYBIND11_MODULE(dislocation_stress, m)
{
    m.doc() = "Triangle dislocation stress and Block ACA library";
    ...
    m.def(
        ...
        py::arg("mu"),
        py::arg("lambda_"),
        "Full-space triangular dislocation stress. Returns an N x 6 NumPy array."
    );
    ...
    m.def(
        ...
        py::arg("mu"),
        py::arg("lambda_"),
        "Half-space triangular dislocation stress. Returns an N x 6 NumPy array."
    );
}
