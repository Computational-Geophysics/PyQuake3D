...
def find_boundary_edges_and_nodes(triangles,node_exclude=[]):
    ...
    # print(boundary_nodes, node_exclude)
    boundary_nodes = boundary_nodes[~np.isin(boundary_nodes, node_exclude)]
...
def outgeomesh(fanme,node,ele):
    Nn=len(node)
    Ne=len(ele)
    fname1=fanme.split('.')[0]
    f=open(fname1+'.msh','w')
    f.write('$MeshFormat\n')
    f.write('2.2 0 8\n')
    ...
    f.write('%d\n'%Nn)
    for i in range(Nn):
        f.write('%d %f %f %f\n'%(i+1,node[i,0],node[i,1],node[i,2]))
    f.write('$EndNodes\n')
    f.write('$Elements\n')
    f.write('%d\n'%Ne)
    for i in range(Ne):
        f.write('%d 2 2 0 1 %d %d %d\n'%(i+1,ele[i,0],ele[i,1],ele[i,2]))
    f.write('$EndElements\n')
...
class QDsim:
    def __init__(self,elelst,nodelst,Para):
        ...
        self.Regularized_fault_zone=0
        self.InputRFZ=False
        #self.halfwidth=self.Para0['Half width']

        self.mpioutdirectory='Outputfiles'
        if not os.path.exists(self.mpioutdirectory):
            os.mkdir(self.mpioutdirectory)


        try:
            self.Regularized_fault_zone=self.Para0['Regularized fault zone']
            self.InputRFZ=self.Para0['InputRFZ']
            self.InputRFZ_file=self.Para0['InputRFZ file']
            self.regeps = np.loadtxt(self.dirname + "/eps.txt")
        ...
        # nodelst0=np.copy(nodelst)/1e3*0.5      
        # outgeomesh('reg.msh',nodelst0,elelst)
        ...
        self.fault_station_index = []
        self.readstation()

        fname=self.mpioutdirectory+'/Init.vtu'
        self.writeVTU(fname,init=True)
        ...
        file = open(self.mpioutdirectory+'/'+self.state_file, "w", encoding="utf-8")

        file.write('Program start time: %s\n'%str(datetime.now()))
        file.write('Input msh geometry file dir:%s\n'%self.dirname)
        file.write('Input parameter file dir:%s\n'%self.dirname)
        ...
        if(self.Regularized_fault_zone>0):
            file.write('Using regularized solution:: True\n')
        else:
            file.write('Using regularized solution:: False\n')

    
    def regularized_2d_kernel(self,y, epsilon):
        """Dimension-reduced regularized kernel evaluated along Y."""

        return (
            3.0 * epsilon**4
            / (4.0 * (y**2 + epsilon**2) ** (5.0 / 2.0))
        )
    ...
    def monitor_total_memory(self,comm, prefix="Total Memory"):
        ...
        if rank == 0:
            ...
            file = open(self.mpioutdirectory+'/'+self.state_file, "a", encoding="utf-8")
    ...
    def start(self):
        ...
        #for i in range(0):
            ...
            if(rank==0):
                ...
                outsteps=int(self.Para0['outsteps'])
                directory=self.mpioutdirectory+'/out_vtu'
                if not os.path.exists(directory):
                    os.mkdir(directory)
                #output slipv and Tt
                if(i%outsteps==0):
                    ...
                    if(self.Para0['outputSLIPV']==True):
                        directory1=self.mpioutdirectory+'/out_slipvTt'
    ...
    def parallel_cells_scatter_send(self,N,nsize=size):
        #N=len(self.eleVec)
    ...
    def calc_greenfuncs_mpi(self):
        ...
        if(rank==0):
            ...
            #         s1=blocks_to_process[i].A.nbytes/(1024*1024)
            #         s2=blocks_to_process[i].B.nbytes/(1024*1024)
            #         s=s+s1+s2
            #     else:
            #         s=s+blocks_to_process[i].Mf_A1s.nbytes/(1024*1024)*6
            # print('memorary:',s)
        ...
        if(self.Lt_jud==False):
            self.local_index=self.parallel_cells_scatter_send(len(self.eleVec),nsize=size)
    ...
    def Init_condition(self):
        ...
        if(self.InputHetoparamter==True):
            ...
            ave_elesize=100
            nodeexclude_indexf=np.where(np.abs(self.nodelst[:,2])<ave_elesize*0.5)[0]

            nodeexclude_index=nodeexclude_indexf+1
            #boundary_edges,boundary_nodes=find_boundary_edges_and_nodes(self.elelst)
            #boundary_edges,boundary_nodes=find_boundary_edges_and_nodes(self.elelst,nodeexclude_index)
        ...
        #self.getpatchs()
        f=open(self.mpioutdirectory+'/Tvalue.txt','w')
    ...
    def read_parameter(self,fname):
        ...
        #print(self.Tno.shape,self.a.shape,self.b.shape)
        self.vec_Tra=np.array([x,y]).transpose()
        self.Tt=(self.Tno-self.P*1e-6)*self.a*np.arcsinh(self.slipv/(2.0*self.V0)*np.exp((self.f0+self.b*np.log(self.V0/self.slipvC))/self.a))
    ...
    def Calc_Tmatrix(self):
        ...
        sourcep=self.regularized_2d_kernel(self.yp[:-1], epsilon=0.1)
        self.sourcep2=sourcep*sourcep
    ...
    def Calc_T_implicit_mpi(self,dt):
        ...
        for i in range(len(self.local_index)):
            ...
            #Ind_term=self.sourcep2*trac[i]*1e6*slipv[i]/self.c
            bv=bv+Ind_term*dt
    ...
    def simu_forward_mpi_LTM(self,dttry):
        ...
        t0 = MPI.Wtime()
        AdotV1,AdotV2,dsigmadt=self.tree_block.blocks_process_MVM(slipv1,slipv2,self.local_blocks)
        if(self.fix_Tn==True):
            dsigmadt=self.normal_loading
        else:
            dsigmadt=dsigmadt+self.normal_loading
        ...
        if(col == 0):
            ...
            for i in range(len(self.fault_station_index)):
                station_index = self.fault_station_index[i]
                matches = np.flatnonzero(self.local_index == station_index)
                #print(rank,i,self.fault_station_index)
                if matches.size > 0:
                    # print(rank,i,self.fault_station_index)
                    local_station_index = int(matches[0])
                    file = open(self.mpioutdirectory+'/'+self.faultstation_names[i], "a")
                    # slip1_value = self.slip1[station_index]
                    # slip2_value = self.slip2[station_index]
                    # slip_value = self.slip[station_index]
                    slipv1_value = self.slipv1[station_index]
                    slipv2_value = self.slipv2[station_index]
                    slipv_value = self.slipv[station_index]
                    Tt1_value = self.Tt1o_local[local_station_index]
                    Tt2_value = self.Tt2o_local[local_station_index]
                    Tno_value = self.Tno_local[local_station_index]
    
                    file.write(f"{self.time:.6f} {slipv1_value:.6e} {slipv2_value:.6e} {slipv_value:.6e} {Tt1_value:.6e} {Tt2_value:.6e} {Tno_value:.6e}\n")
    ...
    def simu_forward_mpi_(self,dttry):
        ...
        # if(len(self.indexTnnegtive)>0):
        #     dVdt1=2*self.Cs/self.mu*AdotV1[self.indexTnnegtive]
        #     dVdt2=2*self.Cs/self.mu*AdotV2[self.indexTnnegtive]
        #     local_index_Tno=self.local_index[self.indexTnnegtive]
        #     slipv1_=np.copy(self.slipv1[local_index_Tno])
        #     slipv2_=np.copy(self.slipv2[local_index_Tno])
        ...
        # if(len(self.indexTnnegtive)>0):
        #     self.slipv1[local_index_Tno]=dVdt1*h+slipv1_
        #     self.slipv2[local_index_Tno]=dVdt2*h+slipv2_
    ...
    # def calc_loading_rate(self):
    #     import greenfunctions_lib.dislocation_lib as dislocation_lib
    #     fvector=[1,0,0]
    #     nu=self.lambda_/(2.0*(self.lambda_+self.mu_))
    #     for i in range(len(self.xg)):
    #         P1=np.copy(self.nodelst[self.elelst[i,0]-1])
    #         P2=np.copy(self.nodelst[self.elelst[i,1]-1])
    #         P3=np.copy(self.nodelst[self.elelst[i,2]-1])

    #         source_p=np.array([P1,P2,P3])

    #         sigma_obs=dislocation_lib.compute_dislocation_stresses(self.xg[i].reshape([1,3]),np.array(source_p),fvector,nu,self.mu_,eps_ratio=0.05)
    #         stress=sigma_obs[:,0,:]
    #         stress=stress.transpose()
    #         stress=stress+stress0
    ...
    def RungeKutte_solve_Dormand_Prince_(self,h):
        ...
        #print('dtau1dt1 ',np.max(dtau1dt1),np.min(dtau1dt1),rank)
        ...
        # self.indexTnnegtive=np.where(Tno_yhk<0)[0]
        # Tt1o_yhk_err[self.indexTnnegtive]=0
        # Tt2o_yhk_err[self.indexTnnegtive]=0
        # Tt1o_yhk[self.indexTnnegtive]=0
        # Tt2o_yhk[self.indexTnnegtive]=0
        ...
        Relerrormax2=np.max(np.abs(Tt1o_yhk_err/(Tt1o_yhk8+1e-10)))
        Relerrormax2o=np.max(np.abs(Tt2o_yhk_err/(Tt2o_yhk8+1e-10)))
    ...
    def writeVTU(self, fname,init=False):
        ...
        if(init==True):
            ...
            if(self.InputRFZ==True):
                print(self.regeps)
                add_scalar("Regularized_fault_zone", self.regeps)
    ...
    def readstation(self):
        station_file = self.dirname + "/station.txt"
        fault_station_file = self.dirname + "/faultstation.txt"
        ...
        def load_file_if_exists(filename):
            ...
            if os.path.exists(filename):
                try:
                    ...
                    faultstation_names=[]
                    faultstation_data=[]
                    f=open(fault_station_file,'r')
                    for line in f:
                        tem=line.split()
                        #print(tem)
                        faultstation_names.append(tem[0])
                        faultstation_data.append([float(tem[1]),float(tem[2]),float(tem[3])])
                    f.close()
                    faultstation_data=np.array(faultstation_data)

                    print(f"Successfully loaded: {filename}")
                    
                    return faultstation_names,faultstation_data

                except ValueError as error:
                    print(f"Cannot read {filename} using np.loadtxt(): {error}")
                    return None
            else:
        ...
        # self.station_data = load_file_if_exists(station_file)
        try:
            self.faultstation_names,self.faultstation_data = load_file_if_exists(fault_station_file)
            # print('!!!!!!!!!!!!!',self.faultstation_names,self.faultstation_data,len(self.faultstation_data))
            ...
            for i in range(len(self.faultstation_data)):
                # print('!!!!!!!!!!!',i,index1_source,self.mpioutdirectory+'/'+self.faultstation_names[i])
                dis=self.xg-self.faultstation_data[i]
                dis=np.linalg.norm(dis,axis=1)
                index1_source = np.argmin(dis)
                self.fault_station_index.append(index1_source)
                file = open(self.mpioutdirectory+'/'+self.faultstation_names[i], "w")
                
                
                
        except:
            print("reading fault station data: failed, please check the file format and content.")
