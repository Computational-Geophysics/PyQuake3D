...
# import greenfunctions_lib.dislocation_analytical_lib as dislocation_analytical_lib
...
def calculate_local_traction(obs, stress):
    """
    Parameters
    ----------
    obs : (N,3,3)
    ...
    strike = np.cross(
        np.broadcast_to(ez, normal.shape),
        normal
    )

    mask = np.linalg.norm(strike, axis=1) < 1e-12
    strike[mask] = np.array([1.0, 0.0, 0.0])

    strike /= np.linalg.norm(strike, axis=1)[:, None]
    ...
    # Build stress tensor
    # ---------------------------------------------------------
    sigma = np.zeros((len(stress), 3, 3))

    sigma[:, 0, 0] = stress[:, 0]   # Sxx
    sigma[:, 1, 1] = stress[:, 1]   # Syy
    sigma[:, 2, 2] = stress[:, 2]   # Szz
    ...
    traction = np.column_stack([
        np.sum(traction_global * strike, axis=1),
        np.sum(traction_global * dip, axis=1),
        np.sum(traction_global * normal, axis=1)
    ])
...
class BlockTree:
    def __init__(self, root_block,nodelst,elelst,eleVec,mu_,lambda_, xg,halfspace_jud, mini_leaf):
        ...
        self.nu_=self.lambda_/(2.0*(self.lambda_+self.mu_))
        ...
        for i in range(len(xg)):
            ...
            tem=np.array([P1,P2,P3])
            truangle_point.append(tem)
        self.truangle_point=np.array(truangle_point)
        self.possonratio=self.lambda_/2.0/(self.lambda_+self.mu_)
        


    def set_reg_fault_zone(self,Para):
        

        self.Regularized_fault_zone=0
        self.InputRFZ=False
        self.InputRFZ_file=None
        try:
            self.Regularized_fault_zone=Para['Regularized fault zone']
            self.InputRFZ=Para['InputRFZ']
            self.InputRFZ_file=Para['InputRFZ file']
        
        except:
            print('No Regularized fault zone parameters in BlockTree')
        ...
        if(self.InputRFZ==True):
            ...
            self.regeps = np.loadtxt(self.dirname + "/"+self.InputRFZ_file)

        #print(self.InputRFZ,self.InputRFZ_file,Para['InputRFZ'],self.regeps)
    ...
    #         source_p.append(tri_tem.reshape([-1]))
    ...
    #                 if(self.InputRFZ==False):
    #                     #print('!!!!!!!!!!!!!!!',self.Regularized_fault_zone)
    #                     sigma_obs=dislocation_lib.compute_dislocation_stresses(self.xg[row_cluster[i]].reshape([1,3]),np.array(source_p),fvector,nu,self.mu_,eps_ratio=self.Regularized_fault_zone)
    #                 else:
    #                     #print('!!!!!!!!!')
    #                     sigma_obs=dislocation_lib.compute_dislocation_stresses_eps(self.xg[row_cluster[i]].reshape([1,3]),np.array(source_p),fvector,nu,self.mu_,eps_ratio=self.regeps[col_cluster])
    #                 stress=sigma_obs[:,0,:]
    ...
    #                 stress=sigma_obs[:,0,:]
    #                 stress=stress.transpose()
    #             #print('stress compare',stress[:10],stress0[:10])
    #             #stress=stress+stress0
    #             #stress=stress0
    ...
    #             else:
    #                 fvector=[1,0,0]
    #                 source_p=np.array([P1,P2,P3]).reshape([1,9])
    #                 nu=self.lambda_/(2.0*(self.lambda_+self.mu_))
    #                 if(self.InputRFZ==False):
    #                     sigma_obs=dislocation_lib.compute_dislocation_stresses(self.xg[row_cluster],source_p,fvector,nu,self.mu_,eps_ratio=self.Regularized_fault_zone)
    #                 else:
    #                     #print('!!!!!!!!!')
    #                     #print(self.regeps[col_cluster[i]],source_p)
    #                     sigma_obs=dislocation_lib.compute_dislocation_stresses_eps(self.xg[row_cluster],source_p,fvector,nu,self.mu_,eps_ratio=[self.regeps[col_cluster[i]]])
    #                 stress=sigma_obs[0]
    #                 stress=stress.transpose()
    #             #stress=stress+stress0
    #             #stress=stress0
    ...
    #         else:
    #             vs_Bs.append(v_k2/(alpha2+1e-10))
    ...
    #         else:
    #             vs_Bd.append(v_k5/(alpha5+1e-10))
    ...
    def calc_greenfunc_dense(self,block, Ss_, Ds_,Ts_):
        col_cluster=block.col_cluster
        row_cluster=block.row_cluster
        X,Y,Z=self.xg[row_cluster, 0], self.xg[row_cluster, 1], self.xg[row_cluster, 2]
        tra_arr=[]
        Ss, Ds, Ts = Ss_, Ds_, Ts_

        if(self.Regularized_fault_zone>0 and self.halfspace_jud==False):
            f_list=np.array([Ss, Ds, Ts ])

            if(self.InputRFZ==False):
                stress = dislocation_lib.compute_dislocation_stresses(
                        self.xg[row_cluster],
                        self.truangle_point[col_cluster],
                        f_list,
                        self.possonratio,
                        self.mu_,
                        eps_ratio=self.Regularized_fault_zone,
                        print_progress=True
                    )
            else:
                stress = dislocation_lib.compute_dislocation_stresses_eps(
                                        self.xg[row_cluster],
                                        self.truangle_point[col_cluster],
                                        f_list,
                                        self.possonratio,
                                        self.mu_,
                                        eps_ratio=self.regeps[col_cluster],
                                        print_progress=True
                                    )
            for i in range(len(col_cluster)):
                tra=calculate_local_traction(self.truangle_point[row_cluster], stress[i])
                tra_arr.append(tra)
            tra_arr=np.array(tra_arr)
                
        else:
            for i in range(len(col_cluster)):

                #print('row_cluster',row_cluster[i])
                P1=self.truangle_point[col_cluster[i],0]
                P2=self.truangle_point[col_cluster[i],1]
                P3=self.truangle_point[col_cluster[i],2]
                if(self.halfspace_jud==True):
                    stress = dislocation_stress.TDstressHS(
                        X,
                        Y,
                        Z,
                        P1,
                    ...
                    stress = dislocation_stress.TDstressFS(
                                        X,
                                        Y,
                                        Z,
                                        P1,
                ...
                tra=calculate_local_traction(self.truangle_point[row_cluster], stress)
                tra_arr.append(tra)
            tra_arr=np.array(tra_arr)
        return tra_arr
    ...
    def ACA_worker(self,block):
        ...
        if(len(block.row_cluster) > self.thread_svd and len(block.col_cluster) > self.thread_svd):
            ...
            options.max_block_rank = -1 # means min(n_block_rows, n_block_cols)
            options.initial_row = 0
            options.pivot_rtol = 1e-10

            
            if(self.Regularized_fault_zone==0):
                acablock = block_aca_lib.compute_triangle_aca_analytical(
                        self.truangle_point[block.row_cluster],
                        self.truangle_point[block.col_cluster],
                        nu=self.nu_,
                        mu=self.mu_,
                        use_half=self.halfspace_jud,
                        options=options
                    )
            else:
                if(self.InputRFZ==False):
                    # print('!!!!!!!!')
                    acablock = block_aca_lib.compute_triangle_aca(
                                            self.truangle_point[block.row_cluster],
                                            self.truangle_point[block.col_cluster],
                                            nu=self.nu_,
                                            mu=self.mu_,
                    ...
                    acablock = block_aca_lib.compute_triangle_aca(
                                            ...
                                            nu=self.nu_,
                                            mu=self.mu_,
                                            eps_ratio=self.regeps[block.col_cluster],
                                            options=options
                                        )
            block.A=np.copy(acablock.A)
            block.B=np.copy(acablock.B)
            ...
            self.size += (block.A.nbytes + block.B.nbytes)/(1024*1024)
            ...
            # block.Mf_Bd=Bd
            Ss_, Ds_,Ts_=1,0,0
            trac_matrixS=self.calc_greenfunc_dense(block, Ss_, Ds_,Ts_)
            Ss_, Ds_,Ts_=0,1,0
            trac_matrixD=self.calc_greenfunc_dense(block, Ss_, Ds_,Ts_)
            block.Mf_A1s=trac_matrixS[:,:,0].transpose()
            block.Mf_A2s=trac_matrixS[:,:,1].transpose()
            block.Mf_Bs=trac_matrixS[:,:,2].transpose()
            block.Mf_A1d=trac_matrixD[:,:,0].transpose()
            block.Mf_A2d=trac_matrixD[:,:,1].transpose()
            block.Mf_Bd=trac_matrixD[:,:,2].transpose()
            self.size +=trac_matrixS.nbytes*2/(1024*1024)
            end_time = time.time()
    ...
    def blocks_process_MVM(self,slip1,slip2,blocks_to_process):
        yvector1=np.zeros(len(slip1))
        yvector2=np.zeros(len(slip1))
        yvector3=np.zeros(len(slip1))
        for i in range(len(blocks_to_process)):
            block=blocks_to_process[i]
            rows = block.row_cluster
            cols = block.col_cluster
            if(hasattr(blocks_to_process[i], 'judaca') and blocks_to_process[i].judaca==True):
                ...
                yvector1[rows] += yout[0::3]
                yvector2[rows] += yout[1::3]
                yvector3[rows] += yout[2::3]
            else:
                s1 = slip1[cols]
                s2 = slip2[cols]
        ...
        return yvector1,yvector2,yvector3
...
class Hmatrix:
    ...
    def init(self,Para0):
        ...
        self.tree_block.set_reg_fault_zone(Para0)
