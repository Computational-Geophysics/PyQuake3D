...
.. list-table:: General Parameters Settings
   ...
   * - Plate loading rate
     - 1 × 10⁻⁹ m/s
     - Plate loading rate.
   * - Regularzed fault zone
     - 0
     - Regularzed fault zone thickness to aviod singularities.
   * - InputRFZ
     - False
     - IF True, import the Input regularized fault zone parameters from an external file.
   * - InputRFZ file
     - -
     - File name of Input regularized fault zone parameters from an external file (one column for each regularzed fault zone thickness, row number is equal to the number of cells).
...
.. code-block:: text
   ...
   staion_name(1) X_coordinate(1) Y_coordinate(1) Z_coordinate(1)
   …
